...
function findLatestCapture(propertyKey, urlKey, viewport) {
  ...
  const files = fs.readdirSync(capturesDir)
		.filter((file) => {
			...
			const pattern = new RegExp(`${propertyKey}-${urlKey}-${viewport}\\.png$`);
...
function processUrl(propertyKey, urlKey) {
  ...
  for (const viewport of property.viewports) {
    const latestCapture = findLatestCapture(propertyKey, urlKey, viewport.name);

    if (latestCapture) {
      copyToControls(latestCapture, propertyKey, urlKey, viewport.name);
    } else {
      console.warn(`No capture found for ${propertyKey}-${urlKey}-${viewport.name}`);
    }
  }
}
...
function main(propertyKey, specificUrl) {
  ...
  if (specificUrl) {
    ...
    if (!property.urls[specificUrl]) {
      console.error(`URL key "${specificUrl}" not found in property "${propertyKey}"`);
      process.exit(1);
    }
